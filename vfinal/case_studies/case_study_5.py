...
def get_distribution():
    ...
    raw_dht11 = observe_throughput(pc_observer_2,
                                   SensorRead(esp_32_1, 'dht11'), 'D:/temp/dht11.log')
    raw_ultrasonic = observe_throughput(pc_observer_3,
                                        SensorRead(esp_32_1, 'ultrasonic'), 'D:/temp/ultrasonic.log')
    raw_rotary_encoder = observe_throughput(pc_observer_4,
                                            SensorRead(esp_32_2, 'rotary_encoder'), 'D:/temp/rotary_encoder.log')
    ...
    ultrasonic_bool = Join(esp_32_3, distance_toggle.out0, distance_filtered.out0, eval_str='False if x else False')  # y if x else False

    joined = Join(esp_32_3, co2_bool.out0, dht11_bool.out0, eval_str='x or y')
    alarm_ser = delay_observer.output(observe_throughput(pc_observer_6,
                                                         Join(esp_32_3, joined.out0, ultrasonic_bool.out0, eval_str='x or y'), 'D:/temp/total.log'))
