...
def get_distribution():
    ...
    selection_int = Map(pc_local, raw_rotary_encoder, eval_str='int(x/2) % 3')

    button_filtered = ButtonFilter(pc_local, raw_button, flip_threshold=1)
    button_single_emit = ButtonToSingleEmit(pc_local, button_filtered)

    joined_selection = Join(pc_local, selection_int, button_single_emit, eval_str='(x, y)')

    duplicator_0 = Duplicate(pc_local, joined_selection)
    duplicator_1 = Duplicate(pc_local, duplicator_0)

    co2_toggle = ToggleState(pc_local, duplicator_0, eval_str='x[0]==0 and x[1]', initial_state=True)
    temperature_toggle = ToggleState(pc_local, duplicator_1, eval_str='x[0]==1 and x[1]', initial_state=True)
    distance_toggle = ToggleState(pc_local, duplicator_1, eval_str='x[0]==2 and x[1]', initial_state=True)

    co2_filtered = Map(pc_local, raw_co2, eval_str='x[2] > 0')
    temperature_filtered = Map(pc_local, raw_dht11, eval_str='x[0] > 45')
    distance_filtered = Map(pc_local, raw_ultrasonic, eval_str='x[0] is not -1 and x[1] < 10')

    co2_bool = Join(pc_local, co2_toggle, co2_filtered, eval_str='y if x else False')
    dht11_bool = Join(pc_local, temperature_toggle, temperature_filtered, eval_str='y if x else False')
    ultrasonic_bool = Join(pc_local, distance_toggle, distance_filtered, eval_str='False if x else False')  # y if x else False

    joined = Join(pc_local, co2_bool, dht11_bool, eval_str='x or y')
    alarm_ser = delay_observer.output(observe_throughput(pc_observer_6,
                                                         Join(pc_local, joined, ultrasonic_bool, eval_str='x or y'), DATA_LOG_PATH + 'total.log'))

    output = PrintQueue(pc_local, alarm_ser, time_frame=100)
